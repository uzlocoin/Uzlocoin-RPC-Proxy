...
function createUzlocoindInstance() {
    console.log("Starting uzlocoind");
    return spawn(config.uzlocoind_exec,
...
function closeUzlocoindByCLI() {
    return new Promise((resolve, reject) => {
        setTimeout(execFile, CREATE_SNAPSHOT_EVERY_MS, config.uzlocoin_cli,
            ['-rpcuser=' + config.rpc_user, '-rpcpassword=' + config.rpc_pass, 'stop'],
            (error, stdout, stderr) => {
                console.log("Stoping uzlocoind");
                if (error) {
                    reject(error);
                }
                resolve();
...
function isUzlocoindStopped(uzlocoindInstance) {
    return new Promise((resolve, reject) => {
        uzlocoindInstance.on('close', (code) => {
            if (code !== 0) {
                console.log("Uzlocoind stopping error", code);
                reject(code);
            }
            console.log("Uzlocoind stopped");
            resolve();
        });
    })
}
...
async function copyData(s3) {
    return new Promise((resolve, reject) => {
        if (!fs.existsSync(config.uzlocoind_data_dir)) {
            reject("Wrong data dir path " + config.uzlocoind_data_dir);
        }

        const s3FilePrefix = getFormattedTime();
        async.every(DIRECTORIES_TO_COPY, (directory, callback) => {
            const body = tar.pack(path.join(config.uzlocoind_data_dir, directory)).pipe(zlib.Gzip());
...
async function updateUzlocoindToTheNewestVersion() {
    ...
    try {
        ...
        // download newest version of available uzlocoind
        const command = 'curl -s https://api.github.com/repos/uzlocoinproject/Uzlocoin/releases/latest \\\n' +
        ...
        if (currentUrl === newUrl) {
            console.log("No newer version of uzlocoind available. The newest is", newUrl);
            return false;
        }

        const downloadCommand = 'wget -O uzlocoin.tar.gz ' + newUrl;
        await execPromise(downloadCommand);

        const tarCommand = 'tar -xzf uzlocoin.tar.gz -C uzlocoind --strip-components=1';
        await execPromise(tarCommand);

        fs.writeFileSync(config.binary_url_file, newUrl);
    }
    catch (e) {
        console.log("Cannot update no new version of uzlocoind");
        console.log(e);
        return false;
    }
...
async function main() {
    try {
        ...
        while (true) {
            let uzlocoindInstance = createUzlocoindInstance();
            await closeUzlocoindByCLI();
            await isUzlocoindStopped(uzlocoindInstance);
            await copyData(s3);
            await updateUzlocoindToTheNewestVersion();
        }
    }
    catch (e) {
        console.log(e);
