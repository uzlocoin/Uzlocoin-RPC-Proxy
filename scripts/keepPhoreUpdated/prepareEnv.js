...
function main() {
    ...
    if (!fs.existsSync(config.uzlocoind_data_dir)) {
        console.log(config.uzlocoind_data_dir, "doesn't exists");
        fs.mkdirSync(config.uzlocoind_data_dir);
        console.log(config.uzlocoind_data_dir, "created");
    }
        ...
        s3.listObjectsV2({Bucket: config.backup_S3_dir, Prefix: s3Prefix.Body.toString()}, (err, objectListInfo) => {
            ...
            async.every(objectListInfo.Contents,
                (objectInfo, callback) => {
                    ...
                    const dirPath = path.join(config.uzlocoind_data_dir, dirName);
